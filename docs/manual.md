...
設定タブでは、表示設定、データ管理、AI診断の設定を行います。

![設定画面](images/15_settings.png)
*設定タブ。表示設定、データ管理、AI設定、バージョン情報が表示されます。*

### 表示設定

不要なタブやフィールドを非表示にして、画面をシンプルにできます。

**タブ表示設定：**
- 💊 処方タブ / 🔬 検査タブの表示/非表示を切り替え
- チェックを外すとタブがナビゲーションから消えます

**フィールド表示設定：**
- 患者情報、カルテ、処方、検査の各セクションごとにフィールドを表示/非表示
- 非表示にしたフィールドは入力フォームに表示されませんが、データは空値で保存されます
- 必須項目（氏名、生年月日、性別、SOAP、薬剤名、検査項目名/値など）は非表示にできません

> **注意:** 表示設定はエクスポート/インポートに含まれます。インポート時に設定も復元されます。全データ削除を実行すると表示設定もリセットされます。
